...
// CAMBIO: Límite de fichas por jugador
const MAX_SELECTIONS_PER_PLAYER = 3;
...
// CAMBIO: Función para verificar si el juego terminó (ahora basada en selecciones totales)
function checkGameOver() {
   // El juego termina cuando algún jugador alcanza 3 selecciones
   for (const userId in gameState.playerSelections) {
       if (gameState.playerSelections[userId].totalSelected >= MAX_SELECTIONS_PER_PLAYER) {
           return true;
       }
   }
   return false;
...
// CAMBIO: Función para inicializar selecciones de un jugador (nueva estructura)
...
async function resetBoardOnly() {
   ...
   // CAMBIO: Reiniciar selecciones de cada jugador
   for (const userId in gameState.playerSelections) {
...
io.on('connection', (socket) => {
   ...
   // CAMBIO: Seleccionar una ficha - Actualizada para el nuevo sistema de 3 fichas
   socket.on('selectTile', async ({ tileIndex, currentScore }) => {
       ...
       // CAMBIO: Verificar si el jugador completó las 3 selecciones
       if (playerSelections.totalSelected >= MAX_SELECTIONS_PER_PLAYER) {
           console.log(`${user.username} ha seleccionado ${MAX_SELECTIONS_PER_PLAYER} fichas, mostrando modal permanente`);
           
           // Marcar que debe mostrar el modal permanente
           playerSelections.showPermanentModal = true;
           
           // Emitir evento para mostrar modal permanente
           ...
           // Guardar estado después de completar las 3 fichas
           await saveGameState();
       ...
       // Para múltiples jugadores, pasar al siguiente turno si no completó las 3 fichas
...
console.log('URL del cliente:', process.env.CLIENT_URL || 'https://juegomemoriacliente3-yk6i.onrender.com');
