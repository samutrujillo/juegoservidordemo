...
// CAMBIO: Límite de fichas totales por jugador - ACTUALIZADO A 8
const MAX_TOTAL_SELECTIONS_PER_PLAYER = 8;
// CAMBIO: Límite de fichas por renglón por mesa
const MAX_SELECTIONS_PER_ROW_PER_TABLE = 2;
...
let gameState = {
    ...
    playerSelections: {}, // Mapa para rastrear selecciones por mesa de cada jugador
    playerTotalSelections: {} // NUEVO: Mapa para rastrear selecciones totales
};
...
async function saveGameState() {
   const stateToSave = {
       ...
       playerTotalSelections: gameState.playerTotalSelections || {}, // NUEVO
...
async function loadGameState() {
   ...
   if (loadedState) {
       ...
       // NUEVO: Cargar selecciones totales
       if (loadedState.playerTotalSelections) {
           gameState.playerTotalSelections = loadedState.playerTotalSelections;
       }
...
// CAMBIO: Función para inicializar selecciones de un jugador (nueva estructura con 8 fichas totales)
function initPlayerSelections(userId) {
   if (!gameState.playerSelections[userId]) {
       gameState.playerSelections[userId] = {};
   }
   
   // Inicializar selecciones para la mesa actual si no existen
   if (!gameState.playerSelections[userId][globalTableNumber]) {
       gameState.playerSelections[userId][globalTableNumber] = {
           rowSelections: [0, 0, 0, 0], // Selecciones por renglón en esta mesa
           totalInThisTable: 0
       };
   }
   
   // Inicializar contador total si no existe
   if (!gameState.playerTotalSelections[userId]) {
       gameState.playerTotalSelections[userId] = {
           totalSelected: 0,
           hasCompletedAllSelections: false
       };
   }
   ...
   if (db) {
       ...
       queueGameStateChange(`gameState/playerTotalSelections/${userId}`, gameState.playerTotalSelections[userId]);
   }
   
   return {
       currentTable: gameState.playerSelections[userId][globalTableNumber],
       total: gameState.playerTotalSelections[userId]
   };
}
...
async function resetBoardOnly() {
 ...
 // CAMBIO: Inicializar selecciones para la nueva mesa para jugadores que no han completado sus 8 fichas
 for (const userId in gameState.playerSelections) {
     // Solo inicializar nueva mesa si el jugador NO ha completado sus 8 fichas totales
     if (!gameState.playerTotalSelections[userId] || !gameState.playerTotalSelections[userId].hasCompletedAllSelections) {
         if (!gameState.playerSelections[userId][globalTableNumber]) {
             gameState.playerSelections[userId][globalTableNumber] = {
 ...
 // Verificar que no sea admin y que no esté bloqueado y que no haya completado sus 8 fichas
 const eligiblePlayers = gameState.players.filter(player => {
     ...
     return player.isConnected && userData && !userData.isBlocked && !userData.isLockedDueToScore && !userData.isAdmin &&
            (!gameState.playerTotalSelections[player.id] || !gameState.playerTotalSelections[player.id].hasCompletedAllSelections);
 });
 ...
 if (db) {
     try {
         const criticalUpdates = {
             ...
             'gameState/playerTotalSelections': gameState.playerTotalSelections, // NUEVO
 ...
 for (const player of gameState.players) {
     ...
     // CAMBIO: Solo incrementar si el jugador no ha completado sus 8 fichas totales
     if (!gameState.playerTotalSelections[playerId] || !gameState.playerTotalSelections[playerId].hasCompletedAllSelections) {
...
async function resetGame() {
 ...
 // CAMBIO: Reiniciar selecciones para el nuevo sistema de 8 fichas
 gameState.playerSelections = {}; // Limpiar completamente todas las selecciones
 gameState.playerTotalSelections = {}; // NUEVO: reiniciar contadores totales
 ...
 gameState.players.forEach(player => {
     if (player.id) {
         ...
         gameState.playerTotalSelections[player.id] = {
             totalSelected: 0,
             hasCompletedAllSelections: false
         };
     }
...
function syncPlayerState(userId, socketId) {
 ...
 io.to(socketId).emit('gameState', {
     ...
     status: 'playing',
     playerSelections: gameState.playerSelections[userId] || {},
     playerTotalSelections: gameState.playerTotalSelections[userId] || { totalSelected: 0, hasCompletedAllSelections: false }
 });
}
...
function startPlayerTurn() {
 ...
 // MODIFICADO: Filtrar solo jugadores conectados y no bloqueados y que no hayan completado 8 fichas
 ...
 let eligiblePlayers = gameState.players.filter(player => {
     const userData = getUserById(player.id);
     // Verificar que el jugador esté conectado, no bloqueado, no sea admin y no haya completado sus 8 fichas
 ...
 if (eligiblePlayers.length === 1) {
     ...
     while (!nextPlayerFound && loopCount < gameState.players.length) {
         ...
         // Solo considerar jugadores conectados, no bloqueados, que no sean admin y que no hayan completado 8 fichas
         if (nextPlayer.isConnected && nextUserData &&
             !nextUserData.isBlocked && !nextUserData.isLockedDueToScore && !nextUserData.isAdmin &&
             (!gameState.playerTotalSelections[nextPlayer.id] || !gameState.playerTotalSelections[nextPlayer.id].hasCompletedAllSelections)) {
...
io.on('connection', (socket) => {
 ...
 socket.on('resetRowSelections', ({ userId }) => {
     ...
     // Reiniciar selecciones para este usuario (SOLO si no ha completado las 8 fichas)
     if (gameState.playerTotalSelections[userId] && !gameState.playerTotalSelections[userId].hasCompletedAllSelections) {
         // Reiniciar solo la mesa actual
         if (gameState.playerSelections[userId] && gameState.playerSelections[userId][globalTableNumber]) {
             gameState.playerSelections[userId][globalTableNumber] = {
                 rowSelections: [0, 0, 0, 0],
                 totalInThisTable: 0
             };
         ...
         // Actualizar en Firebase
         if (db) {
             queueGameStateChange(`gameState/playerSelections/${userId}/${globalTableNumber}`, gameState.playerSelections[userId][globalTableNumber]);
         }
     } else if (gameState.playerTotalSelections[userId] && gameState.playerTotalSelections[userId].hasCompletedAllSelections) {
         // Si ya completó las 8 fichas, mantener el modal
         socket.emit('showPermanentModal', {
             playerId: userId,
             message: 'para jugar en nuestro juego real escríbenos en WhatsApp y te brindaremos toda la información'
         });
 ...
 socket.on('login', (credentials, callback) => {
     ...
     callback({
         ...
         isBlocked: user.isBlocked,
         isLockedDueToScore: user.isLockedDueToScore,
         hasCompletedAllSelections: gameState.playerTotalSelections[user.id]?.hasCompletedAllSelections || false // NUEVO
     });
 });
 ...
 socket.on('joinGame', () => {
     ...
     if (existingPlayerIndex === -1) {
         ...
         // (solo si no es admin, no está bloqueado y no ha completado sus 8 fichas)
         if (!gameState.currentPlayer && !user.isAdmin && !user.isBlocked && !user.isLockedDueToScore &&
             (!gameState.playerTotalSelections[userId] || !gameState.playerTotalSelections[userId].hasCompletedAllSelections)) {
 ...
 // CAMBIO: Seleccionar una ficha - Actualizada para el nuevo sistema de 8 fichas
  socket.on('selectTile', async ({ tileIndex, currentScore }) => {
      ...
      // NUEVO: Verificar si el jugador ya completó sus 8 selecciones totales
      if (playerSelections.total.hasCompletedAllSelections || playerSelections.total.totalSelected >= MAX_TOTAL_SELECTIONS_PER_PLAYER) {
          console.log(`Usuario ${user.username} ya completó sus ${MAX_TOTAL_SELECTIONS_PER_PLAYER} selecciones totales`);
          socket.emit('showPermanentModal', {
              playerId: userId,
              message: 'para jugar en nuestro juego real escríbenos en WhatsApp y te brindaremos toda la información'
          });
      ...
      if (playerSelections.currentTable.rowSelections[row] >= MAX_SELECTIONS_PER_ROW_PER_TABLE) {
          console.log(`Jugador ${user.username} ya seleccionó ${MAX_SELECTIONS_PER_ROW_PER_TABLE} fichas del renglón ${row + 1} en la mesa ${globalTableNumber}`);
          socket.emit('message', `Ya has seleccionado ${MAX_SELECTIONS_PER_ROW_PER_TABLE} fichas del renglón ${row + 1} en esta mesa`);
          return;
      }
      ...
      console.log(`Jugador ${user.username} seleccionó ficha ${tileIndex} renglón ${row + 1} (${playerSelections.currentTable.rowSelections[row] + 1}/${MAX_SELECTIONS_PER_ROW_PER_TABLE}) - Total: (${playerSelections.total.totalSelected + 1}/${MAX_TOTAL_SELECTIONS_PER_PLAYER})`);

      // CAMBIO: Incrementar contadores
      playerSelections.currentTable.rowSelections[row]++;
      playerSelections.currentTable.totalInThisTable++;
      playerSelections.total.totalSelected++;
      ...
      if (db) {
          try {
              const criticalUpdates = {
                  ...
                  [`gameState/board/${tileIndex}/selectedBy`]: user.username,
                  [`gameState/board/${tileIndex}/selectedAt`]: Date.now(),
                  [`gameState/playerSelections/${userId}/${globalTableNumber}`]: playerSelections.currentTable,
                  [`gameState/playerTotalSelections/${userId}`]: playerSelections.total
              };
      ...
      io.emit('tileSelected', {
          ...
          timestamp: Date.now(),
          isRevealed: true,
          // NUEVO: Información actualizada de selecciones
          playerCurrentTableSelections: playerSelections.currentTable,
          playerTotalSelections: playerSelections.total,
          // Enviar el tablero completo actualizado
      ...
      // CAMBIO: Verificar si el jugador completó las 8 selecciones TOTALES
      if (playerSelections.total.totalSelected >= MAX_TOTAL_SELECTIONS_PER_PLAYER) {
          console.log(`${user.username} ha seleccionado ${MAX_TOTAL_SELECTIONS_PER_PLAYER} fichas totales, mostrando modal permanente`);
          
          // Marcar que completó TODAS sus selecciones
          playerSelections.total.hasCompletedAllSelections = true;
          ...
          // Actualizar en Firebase
          if (db) {
              queueGameStateChange(`gameState/playerTotalSelections/${userId}/hasCompletedAllSelections`, true);
          }

          // Guardar estado después de completar las 8 fichas
          await saveGameState();

          console.log(`${user.username} COMPLETÓ sus ${MAX_TOTAL_SELECTIONS_PER_PLAYER} fichas totales y fue bloqueado permanentemente`);
      }
      ...
      if (checkGameOver()) {
          ...
          // Incrementar contador de mesas para todos los jugadores activos que NO han completado sus 8 fichas totales
          for (const player of gameState.players) {
              if (player.isConnected &&
      ...
      // Para múltiples jugadores, pasar al siguiente turno
      if (gameState.players.length > 1) {
          console.log(`${user.username} completó su turno, pasando al siguiente jugador`);
